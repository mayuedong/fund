	...
	go func() {
		...
		for {
			...
			for i, task := range tasks {
				...
				if i < 100 || entity.GetConf().GetForceUpdate() && now.String()[:l] > upTime || now.AddDate(0, 0, task.getWait()).String()[:l] > upTime {
					Download(task)
				} else {
					break
				}
